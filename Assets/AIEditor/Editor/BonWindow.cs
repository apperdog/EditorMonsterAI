...
namespace Assets.Editor
{
	...
	{
    public static BonWindow window;

    private const string Name = "MonsterAI Editor";
		...
		private  Rect _openButtonRect = new Rect(0, 0, 110, TopMenuHeight);
		private  Rect _saveButtonRect = new Rect(110, 0, 110, TopMenuHeight);
		private  Rect _newButtonRect = new Rect(220, 0, 110, TopMenuHeight);
		private  Rect _helpButtonRect = new Rect(330, 0, 80, TopMenuHeight);
		...
		{
			var path = EditorUtility.SaveFilePanel("save graph data", "", "EditorData", "json");
			if (!path.Equals(""))
			{
		...
		{
			if (GUI.Button(_openButtonRect, "Open EditorData"))
			{
				var path = EditorUtility.OpenFilePanel("load editor data", "", "json");
				if (!path.Equals("")) CreateCanvas(path);
			}

			// Save Button
			if (GUI.Button(_saveButtonRect, "Save EditorData")) OpenSaveDialog();
			// New Button
			if (GUI.Button(_newButtonRect, "New EditorData")) CreateCanvas(null);
			// Help Button
