...
namespace Assets.Code.Bon
{
    ...
    public static Graph FromJson(string json)
      ...
      SaveUseData(fileName, graph);

			return true;
		}

    public static void SaveUseData(string fileName, Graph graph)
    {
      fileName = fileName.Insert(fileName.Length - 5, "UseData");
      var file = File.CreateText(fileName);
      file.Write((string)graph.GetUseDataToJson());
      file.Close();
    }
